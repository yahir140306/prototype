...
CREATE TABLE IF NOT EXISTS comentarios (
    ...
    comentario TEXT NOT NULL CHECK (char_length(comentario) >= 10 AND char_length(comentario) <= 500),
...
CREATE POLICY "Cualquiera puede leer comentarios" ON comentarios
    FOR SELECT
    USING (true);

-- Política para insertar comentarios: solo usuarios autenticados
CREATE POLICY "Usuarios autenticados pueden agregar comentarios" ON comentarios
    FOR INSERT
    WITH CHECK (auth.uid() IS NOT NULL AND auth.uid() = user_id);

-- Política para actualizar comentarios: solo el autor puede actualizar
CREATE POLICY "Solo el autor puede actualizar su comentario" ON comentarios
    FOR UPDATE
    USING (auth.uid() = user_id)
    WITH CHECK (auth.uid() = user_id);

-- Política para eliminar comentarios: solo el autor puede eliminar
CREATE POLICY "Solo el autor puede eliminar su comentario" ON comentarios
    FOR DELETE
    USING (auth.uid() = user_id);
...
ADD CONSTRAINT unique_user_cuarto 
UNIQUE (user_id, cuarto_id);
